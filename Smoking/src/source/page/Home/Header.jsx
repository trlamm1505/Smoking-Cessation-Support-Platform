...
const Header = () => {
  ...
  const navItems = ['Home', 'Services', 'Package', 'About', 'Contact'];

  return (
    <header className={`fixed top-0 left-0 w-full z-50 bg-white shadow transition-transform duration-300 ${showHeader ? 'translate-y-0' : '-translate-y-full'}`}>
              ...
              <div className="flex flex-col mt-10 space-y-6">
                {navItems.map((item, idx) => (
                  <a
                    key={idx}
                    href="#"
                    className="font-bold text-gray-900 text-lg hover:text-red-500"
                  >
                    {item}
                  </a>
                ))}
                ...
                <ul className="flex flex-col md:flex-row md:space-x-8 font-medium p-4 md:p-0 mt-4 border rounded-lg md:border-0 bg-white">
                  {navItems.map((item, idx) => (
                    <li key={idx}>
                      <a
                        href="#"
                        className="font-bold block py-2 px-3 text-gray-900 md:p-0 hover:text-red-500"
                      >
                        {item}
                      </a>
                    </li>
